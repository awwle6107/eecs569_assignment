...
* Edited by Hansen to print nicer. 
*                                                                           *
*                                                                           *
...
void stats(double *mtp, double *sdp, int print_bool) {
    ...
    //"[name, size,]'average (us), min, max, stdev,' overhead avg (us), overhead stdev"
	if(print_bool)
		printf("%f,%f,",
		   meantime, sd);
    //printf("\n");
...
void printfooter(char *name, double testtime, double testsd,
	...
	//"[name, size,average (us), min, max, stdev,] 'overhead avg (us), overhead stdev'"
	printf("%f,%f\n",testtime-referencetime,testsd+referencesd);
}
...
#pragma omp master
    ...
    //printf("Running OpenMP benchmark version 3.0 with Hansen MODS!\n"
	//   "\t%d thread(s)\n"
	//   "\t%d outer repetitions\n"
	...
	printf("name, size, average (us), stdev (us), overhead avg (us), overhead stdev (us)\n");
}
...
void finalisereference(char *name) {
    stats(&referencetime, &referencesd, 0);
